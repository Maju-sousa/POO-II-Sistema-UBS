...
from PySide6.QtWidgets import (QMainWindow, QWidget, QHBoxLayout, QVBoxLayout, 
                             ...
                             QGridLayout, QMessageBox, QProgressBar, QComboBox, QDateEdit, QFrame)
from PySide6.QtCore import Qt, Slot, QDate

from interface.estilos import ESTILO_BARRA_LATERAL, ESTILO_GLOBAL, ESTILO_COMPONENTES
from interface.componentes import CardIndicador, CardGenerico, CardConsulta, ModalPaciente, ModalProfissional, ModalUBS, ModalConsulta
...
class MainWindow(QMainWindow):
    ...
    def setup_aba_pacientes(self):
        w, self.grid_pacientes = self.criar_layout_cadastro("Pacientes", "Gerencie os pacientes cadastrados", self.abrir_modal_paciente)
        self.paineis.addWidget(w)

    def setup_aba_profissionais(self):
        w, self.grid_profissionais = self.criar_layout_cadastro("Profissionais", "Gerencie os profissionais de saúde", self.abrir_modal_profissional)
        self.paineis.addWidget(w)

    def setup_aba_ubs(self):
        w, self.grid_ubs = self.criar_layout_cadastro("Unidades Básicas de Saúde", "Gerencie as UBS cadastradas", self.abrir_modal_ubs)
        self.paineis.addWidget(w)
    ...
    def executar_operacao_async(self, acao, *args):
        ...
        self.worker.concluido.connect(self.finalizar_requisicao_async)
    ...
    def finalizar_requisicao_async(self, sucesso, mensagem):
        if not sucesso:
            QMessageBox.critical(self, "Alerta de Validação", mensagem)
        self.atualizar_todas_telas()

    def abrir_modal_paciente(self):
        m = ModalPaciente(self)
        if m.exec() == ModalPaciente.Accepted:
            p = Paciente(m.txt_nome.text(), m.txt_cpf.text(), m.txt_nasc.text(), m.txt_tel.text())
            self.executar_operacao_async(self.gerenciador.adicionar_paciente, p)

    def abrir_modal_profissional(self):
        m = ModalProfissional(self)
        if m.exec() == ModalProfissional.Accepted:
            pr = Profissional(m.txt_nome.text(), m.txt_espe.text())
            self.executar_operacao_async(self.gerenciador.adicionar_profissional, pr)

    def abrir_modal_ubs(self):
        m = ModalUBS(self)
        if m.exec() == ModalUBS.Accepted:
            u = UBS(m.txt_nome.text(), m.txt_end.text())
            self.executar_operacao_async(self.gerenciador.adicionar_ubs, u)
    ...
    def abrir_modal_consulta(self):
        ...
        if m.exec() == ModalConsulta.Accepted:
            ...
            self.executar_operacao_async(self.gerenciador.agendar_consulta, c)
    ...
    def atualizar_todas_telas(self):
        ...
        for i, p in enumerate(self.gerenciador.dados["pacientes"]):
            card = CardGenerico(p["nome"], {"CPF": p["cpf"], "Nascimento": p["nascimento"], "Telefone": p["telefone"]})
            self.grid_pacientes.addWidget(card, i // 2, i % 2)
            
        self.limpar_grid(self.grid_profissionais)
        for i, pr in enumerate(self.gerenciador.dados["profissionais"]):
            card = CardGenerico(pr["nome"], {"Especialidade": pr["especialidade"]})
            self.grid_profissionais.addWidget(card, i // 2, i % 2)

        self.limpar_grid(self.grid_ubs)
        for i, u in enumerate(self.gerenciador.dados["ubs"]):
            card = CardGenerico(u["nome"], {"Endereço": u["endereco"]})
            self.grid_ubs.addWidget(card, i // 2, i % 2)

        self.filtro_paciente.blockSignals(True)
        self.filtro_paciente.clear()
        self.filtro_paciente.addItem("Todos os pacientes", "Todos")
        for p in self.gerenciador.dados["pacientes"]:
            self.filtro_paciente.addItem(p["nome"], p["cpf"])
    ...
    def atualizar_tela_agendamentos(self):
        ...
        for c in self.gerenciador.dados["consultas"]:
            if c["data"] != data_filtro: 
                continue
            if paciente_filtro != "Todos" and c["paciente_cpf"] != paciente_filtro: 
                continue
            consultas_filtradas.append(c)

        for i, c in enumerate(consultas_filtradas):
            nome_p = next((p["nome"] for p in self.gerenciador.dados["pacientes"] if p["cpf"] == c["paciente_cpf"]), "Desconhecido")
            card = CardConsulta(nome_p, c["paciente_cpf"], c["profissional_nome"], c["ubs_nome"], c["data"], c["horario"], c["status"])
            
            card.cancelar_clicked.connect(lambda cp=c["paciente_cpf"], d=c["data"], h=c["horario"]: 
                self.executar_operacao_async(self.gerenciador.alterar_status_consulta, cp, d, h, "Cancelada"))
                
            self.grid_agendamentos.addWidget(card, i // 2, i % 2)
